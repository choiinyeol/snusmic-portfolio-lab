...
- **메인화면**: 30초 안에 현재 포트폴리오 상태, 검토 대기열, 리포트 통계 진입점을 이해시키는 요약 화면.
- **포트폴리오**: 보유/원장/포지션 생애주기/리포트 근거.
- **리포트**: 리포트 발간 후 가격 경로, 목표가 진행률, 적중/실패 통계를 하나의 공유 컬럼 테이블에서 정렬·필터링. 후보 탐색도 이 표의 프리셋/정렬로 처리한다.
- **리포트 통계**: 전체 표본, fat-tail, 경로 고통, 목표가 배수, 사후 보유 질문을 본격적으로 해석한다.
- **전략**: 벤치마크 세트와 고유 원장형 전략을 분리해 수익률, Sharpe, Sortino, MDD, 벤치마크 아웃퍼폼을 비교.
- **가이드**: 데이터 생성 방식, 제한, 용어, 읽기 전용 성격을 설명한다.
...
- Butler screener처럼 정보 밀도가 있지만 정돈된 테이블/필터 UX; 단, 제품 내 별도 screener 라우트는 두지 않고 `/reports`로 통합
...
- shadcn/ui 스타일의 로컬 primitive, Radix, Tailwind 4 같은 검증된 프론트엔드 재료를 활용할 것
- Tailwind v4 CSS-first 구성을 유지하되, 사용하지 않는 UI 플러그인은 의존성에서 제거한다.
- 색상은 Tailwind semantic color family와 CSS 토큰을 일관되게 사용
...
- Tailwind/Radix/local primitive 중심으로 공통 패턴 정리
- custom CSS 줄이기
