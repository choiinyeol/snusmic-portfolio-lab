...
`data/web/report-health.json` is the per-report diagnostics artifact for all 240 source reports. It explains transcription/extraction review, missing prices, and policy-driven web exclusions so the Report Board can show why a report is outside the 217-row visible web sample.
