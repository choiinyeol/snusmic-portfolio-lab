...
`export-web --check` and the web app `artifact:check` must validate cross-references across `reports.json`, `missing-symbols.json`, `manifest.json`, and `data/web/prices/*.json`. Every report/missing symbol needs a matching price artifact. A report symbol may only have a `missing_price=true` artifact when it is also listed in `missing-symbols.json`. If one raw six-digit KRX ticker exports both `.KS` and `.KQ` price artifacts, treat it as a segment-resolution error and fail the gate. The visible report universe must also carry the same `report_id`/`symbol` set across `reports.json`, `reports/table.json`, `report-detail-metrics.json`, `reports/detail-metrics.json`, `return-windows.json`, `reports/return-windows.json`, and `table-download-reports.csv`. Rows that exist in tables/downloads but lack detail metrics or return windows must fail the release gate.
...
`data/web/health.json` is the operational health artifact for the current snapshot's report/price/simulation date alignment and missing-price coverage. Each check must include `ok | review | stale | fail` severity plus observed/expected/action details. `review` may pass with release-gate inspection, but public UI should show availability and dates rather than detailed repair copy. `stale`/`fail` blocks deployment in `artifact:check`. The file must be listed in `manifest.json` and covered by checksum validation.
`data/web/report-health.json` preserves extraction status, web visibility, exclusion reason, and next action for every source report as an internal diagnostics artifact. When a report is absent from `reports.json`, inspect `web_exclusion_reason` in this artifact from admin/release-gate tooling first. Public web pages must not expose those exception rows directly. `missing-symbols.json` must also preserve company/report_id/category/decision/release_status/action so delistings, provider gaps, and mapping fixes pending refresh are distinguishable. `release_status=accepted` is an accepted internal web-sample exclusion warning; `action_required` is the next price-source or mapping repair queue.
