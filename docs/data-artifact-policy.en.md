...
`data/web/report-health.json` preserves extraction status, web visibility, exclusion reason, and next action for every source report. When a report is absent from `reports.json`, inspect `web_exclusion_reason` in this artifact first.
