...
`data/web/health.json` is the operational health artifact for the current snapshot's report/price/simulation date alignment and missing-price coverage. Each check must include `ok | review | stale | fail` severity plus observed/expected/action details. `review` is a UI warning and may pass, while `stale`/`fail` blocks deployment in `artifact:check`. The file must be listed in `manifest.json`, covered by checksum validation, and displayed directly by the web shell Data Status panel.
`data/web/report-health.json` preserves extraction status, web visibility, exclusion reason, and next action for every source report. When a report is absent from `reports.json`, inspect `web_exclusion_reason` in this artifact first. `missing-symbols.json` must also preserve company/report_id/category/decision/release_status/action so delistings, provider gaps, and mapping fixes pending refresh are distinguishable. `release_status=accepted` is an accepted web-sample exclusion warning; `action_required` is the next price-source or mapping repair queue.
