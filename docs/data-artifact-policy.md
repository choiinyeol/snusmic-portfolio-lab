...
`data/web/health.json`은 현재 artifact snapshot의 report/price/simulation 기준일 정렬과 missing-price coverage 상태를 공개하는 운영 health artifact입니다. 이 파일은 `manifest.json`에 포함되어 checksum 검증을 받아야 하며, web shell의 Data Status는 이 값을 그대로 표시해야 합니다.
