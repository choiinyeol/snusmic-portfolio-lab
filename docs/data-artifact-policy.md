...
필요하면 큰 portfolio shard(`data/web/portfolio/equity/*.json`, `data/web/portfolio/daily-decisions/*.json`)는 `manifest.json`의 `external_artifacts` pointer로 외부 mirror/object storage에 둘 수 있습니다. 이 경우 index/page/summary artifact는 계속 `data/web`에 남고, web build 전에 `apps/web/scripts/hydrate-external-artifacts.mjs`가 local cache로 가져오거나, 공개 URL이 있으면 그 URL을 기준으로 hydrate해야 합니다. 커밋되는 external pointer는 deploy 경로가 닫히도록 `public_url`을 가져야 합니다.
`export-web --check`와 `apps/web`의 `artifact:check`는 `reports.json`, `missing-symbols.json`, `manifest.json`, `data/web/prices/*.json`의 cross-reference를 검증해야 합니다. 모든 report/missing symbol은 대응 price artifact를 가져야 하고, report symbol의 `missing_price=true` artifact는 `missing-symbols.json`에 기록된 symbol에만 허용됩니다. 동일한 6자리 KRX ticker가 `.KS`와 `.KQ` 양쪽 price artifact로 동시에 export되면 segment resolver 오류로 보고 실패해야 합니다. 또한 visible report universe는 `reports.json`, `reports/table.json`, `report-detail-metrics.json`, `reports/detail-metrics.json`, `return-windows.json`, `reports/return-windows.json`, `table-download-reports.csv`에서 동일한 `report_id`/`symbol` 집합을 유지해야 합니다. 테이블·다운로드에는 있는데 상세 metric이나 return-window가 없는 row는 release gate 실패입니다.
...
`data/web/health.json`은 현재 artifact snapshot의 report/price/simulation 기준일 정렬과 missing-price coverage 상태를 보존하는 운영 health artifact입니다. 각 check는 `ok | review | stale | fail` severity, observed/expected/action을 포함해야 합니다. `review`는 release gate에서 확인할 수 있지만, public UI는 상세 조치 문구 대신 사용 가능 여부와 기준일만 보여줍니다. `stale`/`fail`은 `artifact:check`에서 배포 차단으로 처리합니다. 이 파일은 `manifest.json`에 포함되어 checksum 검증을 받아야 합니다.
`data/web/report-health.json`은 240개 원천 리포트 전체의 전사/추출 상태, 웹 노출 여부, 제외 사유, 다음 조치를 보존하는 내부 진단 artifact입니다. 리포트가 `reports.json`에 없을 때는 admin/release-gate 점검에서 이 artifact의 `web_exclusion_reason`을 먼저 확인해야 합니다. Public web page는 이 제외·오류 행을 직접 노출하지 않습니다. `missing-symbols.json`은 symbol뿐 아니라 company/report_id/category/decision/release_status/action을 함께 보존해 상장폐지, provider gap, mapping 보정 대기 상태를 구분해야 합니다. `release_status=accepted`는 표본 제외를 수용한 내부 경고이고, `action_required`는 다음 가격 소스 또는 mapping 보정 큐입니다.
