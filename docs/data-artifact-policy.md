...
`data/web/report-health.json`은 240개 원천 리포트 전체의 전사/추출 상태, 웹 노출 여부, 제외 사유, 다음 조치를 보존합니다. 리포트가 `reports.json`에 없을 때는 이 artifact에서 `web_exclusion_reason`을 먼저 확인해야 합니다. `missing-symbols.json`은 symbol뿐 아니라 company/report_id/category/decision/release_status/action을 함께 보존해 상장폐지, provider gap, mapping 보정 대기 상태를 구분해야 합니다. `release_status=accepted`는 표본 제외를 수용한 경고이고, `action_required`는 다음 가격 소스 또는 mapping 보정 큐입니다.
