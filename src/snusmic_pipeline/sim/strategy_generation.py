...
from .broker_strategy_search import (
    BrokerStrategySearchResult,
    _strategy_display_label,
    find_top_broker_strategy_configs,
)
...
def run_strategy_generation(config: StrategyGenerationConfig) -> GenerationResult:
    ...
    summary = {
        ...
        "pit_promoted": [p.persona_name for p in pit_promoted],
        "broker_promoted": [p.persona_name for p in broker_search.configs],
        "rejected_by_portfolio_benchmark": sorted(
            set(stock_gate.rejected_by_benchmark) | set(pit_gate.rejected_by_benchmark)
        ),
        "rejected_by_correlation": sorted(
            set(stock_gate.rejected_by_correlation) | set(pit_gate.rejected_by_correlation)
        ),
        "mtt_note": (
            ...
            "Promotion now treats benchmark return as comparative evidence rather than a hard gate."
        ),
    }
    ...
    return GenerationResult(
        ...
        pit_promoted=tuple(p.persona_name for p in pit_promoted),
        broker_promoted=tuple(p.persona_name for p in broker_search.configs),
        rejected_by_portfolio_benchmark=tuple(
            sorted(set(stock_gate.rejected_by_benchmark) | set(pit_gate.rejected_by_benchmark))
        ),
...
def select_benchmark_summary(
    ...
    return BenchmarkSelection(
        ...
        money_weighted_return=float(benchmark.money_weighted_return),
    )
...
def _load_reusable_stock_trials(out_dir: Path) -> pd.DataFrame | None:
    """Reuse immutable historical stock-rule replay metrics when explicitly enabled.

    The stock-rule validation replay is the slowest deterministic stage.  When a
    change only alters admission policy, not historical prices/reports or rule
    formulas, the existing replay rows can be reused and re-gated.  This fast
    path intentionally requires an environment flag so normal scheduled runs
    still recompute from source data unless an operator opts in.
    """

    if os.environ.get("SNUSMIC_REUSE_STOCK_RULE_ADMISSION", "").lower() not in {"1", "true", "yes"}:
        return None
    path = out_dir / "validation-admission.csv"
    if not path.exists():
        return None
    frame = pd.read_csv(path)
    if frame.empty or "rule_id" not in frame.columns:
        return None
    if "admission_status" in frame.columns:
        statuses = frame["admission_status"].astype(str)
        benchmark_only = statuses.str.contains("benchmark", case=False, na=False)
        if "admission_reasons" in frame.columns:
            benchmark_only = benchmark_only | frame["admission_reasons"].astype(str).str.contains(
                "benchmark", case=False, na=False
            )
            frame.loc[benchmark_only, "admission_reasons"] = "[]"
        hard_rejected = (
            statuses.str.contains("activity", case=False, na=False)
            | statuses.str.contains("duplicate", case=False, na=False)
            | statuses.str.contains("correlation", case=False, na=False)
        )
        frame["accepted"] = ~hard_rejected
    else:
        frame["accepted"] = True
    frame.attrs["reused_validation_admission"] = True
    return frame


def broker_strategy_search_cached(
    config: StrategyGenerationConfig,
    base_config: SimulationConfig,
    benchmark_money_weighted_return: float,
) -> BrokerStrategySearchResult:
    """Reuse fixed broker-strategy trial ledgers when only admission policy changed."""

    if not config.refresh_benchmark and os.environ.get(
        "SNUSMIC_DISABLE_BROKER_STRATEGY_REUSE", ""
    ).lower() not in {"1", "true", "yes"}:
        reused = _load_reusable_broker_strategy_search(config)
        if reused is not None:
            return reused

    return find_top_broker_strategy_configs(
        warehouse_dir=config.warehouse_dir,
        ...
        benchmark_money_weighted_return=benchmark_money_weighted_return,
...
def _load_reusable_broker_strategy_search(
    config: StrategyGenerationConfig,
) -> BrokerStrategySearchResult | None:
    path = config.out_dir / "broker_strategy_trials.csv"
    if not path.exists():
        return None
    frame = pd.read_csv(path)
    required = {
    ...
    for row in updated.to_dict("records"):
        ...
        statuses.append(
            "accepted" if accepted else "duplicate_behavior" if duplicate_behavior else "below_risk_gate"
        )
        if accepted:
            seen_behavior_keys.add(behavior_key)

    updated["accepted"] = accepted_flags
    updated["admission_status"] = statuses

    configs: list[SmicMttStrategyConfig] = []
    for rank, row in enumerate(updated[updated["accepted"]].to_dict("records"), start=1):
        base_payload = {field: _none_if_nan(row.get(field)) for field in SmicMttStrategyConfig.model_fields}
        base_payload["persona_name"] = f"smic_mtt_strategy_top{rank}"
        base_payload["source_trial_number"] = int(
            row.get("trial_number") or row.get("source_trial_number") or 0
        )
        base_payload["selection_rank"] = int(row.get("train_rank") or rank)
        base_payload["train_money_weighted_return"] = float(row.get("train_money_weighted_return") or 0.0)
        draft = SmicMttStrategyConfig.model_validate(base_payload)
        configs.append(draft.model_copy(update={"label": _strategy_display_label(draft, rank)}))
        if config.broker_strategy_top > 0 and len(configs) >= config.broker_strategy_top:
            break

    return BrokerStrategySearchResult(configs=tuple(configs), trial_rows=updated)
...
def portfolio_gate(
    *,
    ...
    selected, gate = portfolio_gate_personas(
        ...
        benchmark_persona=benchmark_persona,
        benchmark_money_weighted_return=benchmark_money_weighted_return,
        max_correlation=max_correlation,
    )
    return tuple(p for p in selected if isinstance(p, StockRulePersonaConfig)), gate


def portfolio_gate_personas(
    *,
    config: StrategyGenerationConfig,
    base_personas: tuple[PersonaConfig, ...],
    candidates: tuple[PersonaConfig, ...],
    benchmark_persona: str,
    benchmark_money_weighted_return: float,
    max_correlation: float,
) -> tuple[tuple[PersonaConfig, ...], PortfolioGateResult]:
    """Approve generated personas by final ledger evidence and behavior diversity."""
    if not candidates:
    ...
    result = run_simulation_cached(
        config, sim_config, stage=f"portfolio-gate-{_gate_cache_suffix(candidates)}"
    )
    summaries = {summary.persona: summary for summary in result.summaries}
    evaluated: list[PersonaConfig] = []
    metrics_by_rule_id: dict[str, dict[str, Any]] = {}
    ...
    returns = equity_return_series(result, [candidate.persona_name for candidate in evaluated])
    ranked = sorted(
        evaluated,
        key=lambda candidate: (
            metrics_by_rule_id.get(_candidate_key(candidate), {}).get("portfolio_money_weighted_return")
            or -1e9,
            metrics_by_rule_id.get(_candidate_key(candidate), {}).get("portfolio_sharpe") or -1e9,
    ...
    for candidate in ranked:
        ...
        if peer is not None and peer_corr >= max_correlation:
            corr_rejected.add(candidate_key)
            corr_peer[candidate_key] = peer
            metrics_by_rule_id.setdefault(candidate_key, {})["portfolio_max_correlation"] = peer_corr
            continue
        selected.append(candidate)
        if series.size:
            selected_series.append((candidate_key, series))
    return tuple(selected), PortfolioGateResult(
        accepted_rule_ids=frozenset(_candidate_key(candidate) for candidate in selected),
...
def annotate_stock_trial_gate(frame: pd.DataFrame, gate: PortfolioGateResult) -> pd.DataFrame:
    ...
    for row in updated.to_dict("records"):
        ...
        if rule_id in gate.accepted_rule_ids:
            ...
            accepted.append(False)
            previous_status = str(row.get("admission_status") or "not_selected")
            statuses.append(
                "not_selected"
                if previous_status == "accepted" or "benchmark" in previous_status
...
def write_pit_outputs(
    config: StrategyGenerationConfig,
    candidates: tuple[PitResearchBoardConfig, ...],
    promoted: tuple[PitResearchBoardConfig, ...],
    gate: PortfolioGateResult,
    benchmark_money_weighted_return: float,
) -> None:
    """Write PIT research-board strategy configs and human-auditable lineage rows."""
    config.out_dir.mkdir(parents=True, exist_ok=True)
    admission_rows = []
    for candidate in candidates:
        ...
        admission_rows.append(
            {
                ...
                "accepted": key in gate.accepted_rule_ids,
                "admission_status": status,
                "benchmark_money_weighted_return": benchmark_money_weighted_return,
                "portfolio_correlated_with_rule_id": gate.correlation_peer.get(key),
                **metrics,
                **{f"config_{k}": v for k, v in candidate.model_dump(mode="json").items()},
            }
        )
    admission = pd.DataFrame(admission_rows)
    _write_rows(
        admission,
        config.out_dir / "pit-research-board-admission.csv",
        config.out_dir / "pit-research-board-admission.json",
    )
    (config.out_dir / "pit-research-board-personas.json").write_text(
        json.dumps([_json_safe(p.model_dump(mode="json")) for p in promoted], ensure_ascii=False, indent=2)
        + "\n",
        encoding="utf-8",
    )
    ...
    if promoted:
        reports, board, trading_dates = _pit_snapshot_inputs(config)
        for persona_config in promoted:
            snapshot_records.extend(snapshot_rows_for_config(persona_config, reports, board, trading_dates))
    ...
    _write_rows(
        snapshot_frame,
        config.out_dir / "pit-research-board-snapshots.csv",
        config.out_dir / "pit-research-board-snapshots.json",
    )


def _pit_snapshot_inputs(config: StrategyGenerationConfig) -> tuple[pd.DataFrame, PriceBoard, list[date]]:
    board = PriceBoard.from_warehouse(config.warehouse_dir)
    reports = read_table(config.warehouse_dir, "reports")
    ...
    return reports, board, trading_dates
...
def run_simulation_cached(
    ...
    *,
    ...
    """Run a deterministic simulation with a disk cache keyed by config and warehouse mtimes.

    Strategy gate changes often only re-score fixed historical ledgers.  Caching
    prevents repeatedly replaying the same immutable report/price history while
    still invalidating when the warehouse files, date range, personas, or
    benchmark refresh setting changes.
    """

    if config.refresh_benchmark or os.environ.get("SNUSMIC_DISABLE_STRATEGY_SIM_CACHE", "").lower() in {
        "1",
        "true",
        "yes",
    }:
        return run_simulation(sim_config, config.warehouse_dir, refresh_benchmark=config.refresh_benchmark)

    payload = {
        "schema_version": "1.0.0",
        "stage": stage,
        "simulation_config": sim_config.model_dump(mode="json"),
        "warehouse": _warehouse_fingerprint(config.warehouse_dir),
        "refresh_benchmark": config.refresh_benchmark,
    }
    digest = hashlib.sha256(json.dumps(payload, ensure_ascii=False, sort_keys=True).encode()).hexdigest()[:20]
    cache_dir = config.out_dir / ".cache" / "strategy-generation"
    cache_path = cache_dir / f"{_safe_cache_name(stage)}-{digest}.json"
    if cache_path.exists():
        return SimulationResult.model_validate_json(cache_path.read_text(encoding="utf-8"))
...
def _warehouse_fingerprint(warehouse_dir: Path) -> dict[str, dict[str, int | str]]:
    ...
    for name in files:
        path = warehouse_dir / name
        if not path.exists():
            fingerprint[name] = {"missing": "1"}
            continue
        stat = path.stat()
        fingerprint[name] = {"size": int(stat.st_size), "mtime_ns": int(stat.st_mtime_ns)}
    return fingerprint
...
def _gate_cache_suffix(candidates: tuple[PersonaConfig, ...]) -> str:
    if not candidates:
        return "empty"
    first = _candidate_key(candidates[0])
    return f"{len(candidates)}-{first}"


def _candidate_key(candidate: PersonaConfig) -> str:
    if isinstance(candidate, StockRulePersonaConfig):
        return candidate.rule_id
    return candidate.persona_name
