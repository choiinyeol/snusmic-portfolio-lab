      ...
      >
        <td className="tnum whitespace-nowrap px-1.5 py-1 font-mono text-xs text-muted-foreground">
          {expandable ? (
            <button
              type="button"
              onClick={() => onToggle(report.source_name)}
              aria-expanded={open}
...
function PaginationBar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 border-t border-dashed border-border px-3 py-2.5">
      <span className="mr-1 font-mono text-[10px] text-muted-foreground">페이지:</span>
...
export function ReportLedger({
  ...
  const totalPages = sort.sorted.length > PAGE_THRESHOLD ? Math.ceil(sort.sorted.length / PAGE_SIZE) : 1;
  const pageRows = totalPages > 1 ? sort.sorted.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE) : sort.sorted;

  // Stable toggle callback — does not change on re-render
  const handleToggle = useCallback((sourceName: string) => {
    setExpandedName((prev) => (prev === sourceName ? null : sourceName));
  }, []);

  // Reset page and collapse when reference filter or sort changes
  ...
  // Wrap sort.toggle to also reset page
  const sortToggle = useCallback(
    (key: string, multi: boolean) => {
      sort.toggle(key, multi);
      setPage(0);
    },
  ...
  const sortState = useMemo(
    () => ({ specs: sort.specs, toggle: sortToggle }),
    [sort.specs, sortToggle],
  );
        ...
        <table className="ledger-table w-full min-w-[1320px] border-collapse text-sm">
          <thead>
            <tr className="border-b-4 border-double border-border font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
              <SortableTh sortKey="date" sort={sortState} title="발간일 — ▸ 를 누르면 가격 경로가 펼쳐집니다">발간일</SortableTh>
              {showSchool && <SortableTh sortKey="school" sort={sortState}>학회</SortableTh>}
              <SortableTh sortKey="name" sort={sortState}>종목</SortableTh>
              <SortableTh sortKey="rating" sort={sortState}>의견</SortableTh>
              <SortableTh sortKey="price" sort={sortState} align="right">발간가</SortableTh>
              <SortableTh sortKey="target" sort={sortState} align="right">목표가</SortableTh>
              <SortableTh sortKey="upside" sort={sortState} align="right" title="발간 당시 제시한 상승여력">괴리율</SortableTh>
              <SortableTh sortKey="r90" sort={sortState} align="right">3M</SortableTh>
              <SortableTh sortKey="r365" sort={sortState} align="right">1Y</SortableTh>
              <SortableTh sortKey="latest" sort={sortState} align="right">발간이후</SortableTh>
              <SortableTh sortKey="alpha" sort={sortState} align="right" title="같은 기간 시장지수 대비 초과수익">알파</SortableTh>
              <SortableTh sortKey="peak" sort={sortState} align="right" title="발간 후 24개월 내 장중 최고가 수익률 — 보고서의 전성기">전성기</SortableTh>
              <SortableTh sortKey="bucket" sort={sortState} align="right">등급</SortableTh>
              <SortableTh sortKey="days" sort={sortState} align="right" title="목표가 도달까지 걸린 시간">판결</SortableTh>
            </tr>
          </thead>
          <tbody>
            {pageRows.map((report) => (
              <LedgerRow
                ...
                linkStocks={linkStocks}
                open={expandedName === report.source_name}
