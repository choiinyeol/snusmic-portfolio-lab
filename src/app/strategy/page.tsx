...
export default function StrategyPage() {
  ...
  // 연구 기록 테이블 가지치기 (v18): L/M은 백테스트에서 제거됨(행 없음).
  // S 비베스트 변형·V_ls 대조군은 방법론/SPO 섹션에서 다루므로 표에서 숨김.
  const sVariantKeys = ["S_hrp", "S_msharpe", "S_mincvar"];
  const hiddenResearchKeys = new Set([...sVariantKeys.filter((k) => k !== bestSKey), "V_ls"]);
  const researchRows = multiStrategy.strategies.filter((r) => !hiddenResearchKeys.has(r.key));
    ...
    <main className="mx-auto max-w-[1500px] space-y-8 px-4 py-6 sm:px-8">
      ...
      <details className="group rounded-lg border border-border bg-card">
        ...
        <div className="border-t border-border px-5 pb-5 pt-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            표에서 생략: S 비베스트 배분 변형 2종(방법론 §S 참조), V-b LS 대조군(SPO 방법론 공개 참조).
            L 민리버전(RSI-2)·M 단기 리버설은 룩어헤드·버그 수정 후 재검증에서도 거래비용 사망이 확정되어
            v18부터 백테스트에서 제거 — 테스트했고 실패했다는 기록만 남깁니다.
          </p>
        </div>
      </details>
