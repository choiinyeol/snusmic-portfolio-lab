...
- Next.js: `data/web` 기준 데이터를 읽는 정적 스냅샷/포트폴리오/리포트 검증/전략 비교 대시보드
...
- 핵심 화면: 스냅샷, 포트폴리오, 리포트 검증, 전략 비교, 후보 탐색, 읽는 법
- 핵심 UI 원칙: 표는 기본적으로 정렬·필터·페이지네이션을 제공하고, 같은 행 데이터를 다루는 관심별 뷰는 하나의 공유 컬럼 테이블에서 정렬/필터 프리셋만 바꿉니다.
- 키보드·접근성 원칙: `/`로 표 검색 입력에 즉시 포커스하고 `Esc`로 비웁니다. 모든 페이지는 "본문으로 건너뛰기" 링크와 `loading.tsx` 스켈레톤을 갖추고, 사이드바 펼침 상태는 `localStorage`에 보존됩니다. 정렬 헤더는 `aria-sort`, 필터 결과 수는 `aria-live`로 스크린리더에 노출합니다.
...
# run-sim은 후보별 accepted/below_benchmark/duplicate_behavior 상태를 broker_strategy_trials.csv로 남깁니다.
...
- `broker_strategy_trials.csv`: broker-ledger 전략 후보의 채택/탈락 사유와 성과 지표

### `data/web`

Next.js의 1차 reader(`apps/web/lib/artifacts.ts`)는 page-owned bundle을 읽습니다.
상위 파일(`overview.json`, `reports.json`, `personas.json` 등)은 exporter 호환성,
다운로드, 테스트용으로 유지되지만 프론트엔드의 주 경로가 아닙니다.

- `strategies/admission.json` / `strategy-admission.json`: 왜 현재 고유 전략 몇 개만 채택됐는지 설명하는 후보 admission 감사 trail
- `prices/{symbol}.json`: canonical OHLCV와 함께 split/reverse-split 이벤트가 있는 종목에 한해 `stock_split`, `split_event_type`, `split_ratio_text`, `cum_split_factor_to_latest`, `split_adjusted_*` 진단 필드를 포함합니다.

- `manifest.json`: schema version, generated timestamp, date ranges, row counts, checksums
- `overview/snapshot.json`, `overview/research-pulse.json`, `overview/data-quality.json`: 스냅샷·리서치 펄스·품질 요약
- `reports/table.json`, `reports/rankings.json`, `reports/detail-metrics.json`, `reports/return-windows.json`, `reports/target-hit-distribution.json`: 리포트 검증 화면 데이터
- `portfolio/personas.json`, `portfolio/holdings.json`, `portfolio/monthly-holdings.json`, `portfolio/trades.json`, `portfolio/episodes.json`, `portfolio/equity-daily.json`: 원장형 포트폴리오 데이터
- `strategies/catalog.json`, `strategies/leaderboard.json`, `strategies/curves.json`: 전략 taxonomy·성과·차트 데이터
- `screener/candidates.json`: 리포트 기반 후보 탐색 데이터
- `prices/*.json`: 심볼별 가격 차트 데이터. yfinance `Stock Splits` 이벤트를 `stock_split`, `split_event_type`, `split_ratio_text`, `split_factor`, `cum_split_factor_to_latest`, `split_adjusted_*` 필드로 보존해 액면분할/액면병합 보정 원인을 추적할 수 있습니다.
