...
|---|---:|---:|---:|---:|---:|---:|
| **Weak Prophet** (6M 룩어헤드 max-Sharpe) | 3,002.99M | 1억 | **+2,902.99M** | **150.44%** | 27.4% | 1,134 |
| **Prophet** (Full 룩어헤드 max-Sharpe) | 439.30M | 1억 | +339.30M | 58.81% | 13.4% | 1,184 |
| **All-Weather** (벤치마크) | 217.83M | 1억 | +117.83M | 30.54% | 9.6% | 211 |
| **SMIC Follower v2** (손절 추가) | 181.20M | 1억 | +81.20M | 23.29% | 22.4% | 1,904 |
| **SMIC Follower** (1/N 충신) | 139.01M | 1억 | +39.01M | 12.91% | 15.2% | 3,946 |

> 흥미로운 발견: **Prophet (full-horizon max-Sharpe) 이 Weak Prophet (6M max-Sharpe) 에 진다**. 더 많은 정보를 가진 페르소나가 적은 정보의 페르소나보다 못 번 이유는 max-Sharpe 자체가 근시안적이기 때문입니다 — 5년치 평균 수익률/공분산은 6개월 평균보다 smoothed out 되어 단기 winner를 못 잡습니다. "정보가 많은 게 항상 좋은 건 아니다"는 정량적 증거.
>
> 실거래 가능성 측면: Prophet은 매월 max-Sharpe 풀-룩어헤드 포트폴리오로 리밸런스, max_weight=0.40 으로 분산 → 거래 가능한 수량의 4-6 종목 basket. 158억주 매수 같은 비현실적 행동 없음 (이전 "1등 종목 100%" 버전 대비 정상화).
>
...
#### Prophet (보유 0종목, 전액 cash 439.30M KRW)

마지막 리밸런스 (2026-04-01) 시점에 future 윈도가 14거래일밖에 안 남아 max-Sharpe 솔버의 `min_history_days=60` 컷오프에 걸려 weights 가 비어버렸고, 결과적으로 모든 포지션을 청산했습니다. 시뮬 종료 시 전액 현금 보유, 실현이익 339.30M.

#### Weak Prophet (총 22종목, 평가금 합 2,997.0M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| NE | Noble Corporation PLC | 6,032 | 41,850 | 69,680 | 420.3 | +66.0% |
| TS | Tenaris S.A. | 4,047 | 52,608 | 85,065 | 344.3 | +62.0% |
| 353810.KQ | 이지바이오 | 41,185 | 5,823 | 8,020 | 330.3 | +38.0% |
| 278470.KS | 에이피알 | 775 | 181,479 | 413,000 | 320.1 | +128.0% |
| STNG | Scorpio Tankers Inc. | 2,769 | 75,789 | 108,678 | 300.9 | +43.0% |

#### All-Weather (총 4종목, 평가금 합 217.8M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| 069500.KS | KOSPI 200 ETF | 614 | 82,223 | 92,665 | 56.9 | +13.0% |
| QQQ | NASDAQ-100 ETF | 59 | 862,596 | 936,608 | 55.3 | +9.0% |
| SPY | S&P 500 ETF | 53 | 960,760 | 1,028,506 | 54.5 | +7.0% |
| GLD | Gold ETF | 79 | 548,583 | 647,221 | 51.1 | +18.0% |

#### SMIC Follower v2 (총 17종목, 평가금 합 181.2M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| 122640.KS | 예스티 | 498 | 20,413 | 29,200 | 14.5 | +43.0% |
| 017960.KS | 한국카본 | 247 | 41,177 | 51,100 | 12.6 | +24.0% |
| 000150.KS | 두산 | 9 | 1,113,724 | 1,358,000 | 12.2 | +22.0% |
| 294870.KQ | HDC현대산업개발 | 475 | 21,414 | 23,500 | 11.2 | +10.0% |
| 234300.KQ | 에스트래픽 | 3,023 | 3,367 | 3,590 | 10.9 | +7.0% |

#### SMIC Follower (1/N) (총 100종목, 평가금 합 136.2M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| 228670.KS | 레이 | 242 | 5,263 | 10,720 | 2.6 | +104.0% |
| 368600.KS | 아이씨에이치 | 1,427 | 893 | 1,468 | 2.1 | +64.0% |
| CRWV | Coreweave | 11 | 119,630 | 174,405 | 1.9 | +46.0% |
| CHGG | Chegg | 1,177 | 1,044 | 1,602 | 1.9 | +53.0% |
| 159010.KS | 아스플로 | 229 | 5,544 | 7,990 | 1.8 | +44.0% |
...
### 3-1. `oracle` — Prophet (Full 룩어헤드 max-Sharpe)

Weak Prophet과 **같은 max-Sharpe 알고리즘**을 사용하되, 룩어헤드 윈도를 시뮬 끝까지로 확장한 페르소나.

- 매월 첫 영업일에 활성 SMIC 리포트 종목별로 **오늘 ~ 시뮬 종료일** 의 실현 일별 수익률을 계산
- `scipy.optimize.minimize` (SLSQP) 로 long-only max-Sharpe 포트폴리오 (sum=1, 종목당 캡 `max_weight=0.40`, 무위험금리 3%)
- 결과 비중대로 share book 리밸런스

이전 버전 (1개월 lookahead + 100% 집중) 은 Prophet 누적 +21억 → 이건 158억주 같은 비현실적 매수가 발생했습니다. 본 버전은 max_weight=0.40 으로 분산되어 4-6 종목 basket 으로 운영됩니다 (실거래 가능 범위).

**예상치 못한 결과**: Prophet 이 Weak Prophet 에 집니다. 5년치 평균 수익률/공분산이 6개월 윈도보다 smoothed out 되어 단기 winner를 못 따라잡기 때문 — max-Sharpe 자체가 myopic 한 결과.
...
- yfinance 가격 데이터는 `auto_adjust=False` 로 받아옵니다 — 즉 SMIC 보고서가 인용한 실제 시장가와 일치하는 raw OHLC 입니다. 그 결과 시뮬은 **배당 재투자 수익을 포함하지 않으니** 실제 투자자 누적 수익률은 여기 표시된 것보다 약간 더 높을 수 있습니다 (특히 고려신용정보처럼 고배당 종목 비중이 큰 페르소나).
