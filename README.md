...
|---|---:|---:|---:|---:|---:|---:|
| **Prophet** (예언자, 상한선) | 1,785,070,772M | 1억 | **+1,785,070,672M** | **2,268.66%** | 42.9% | 3,256 |
| **Weak Prophet** (6M 룩어헤드) | 3,002.99M | 1억 | +2,902.99M | 150.44% | 27.4% | 1,134 |
...
> Prophet은 매월 리밸런스 시 **다음 한 달간 가장 많이 오를 종목**을 100% 비중으로 잡고 복리시키는 구조입니다 (이전 버전은 lifetime peak에서 일괄 청산 후 현금 보유 → underperform 이슈가 있었음). 이게 "절대 도달 불가능한 상한선"의 정의입니다. 실제 사람의 결과는 SMIC Follower ~ Weak Prophet 사이에 위치합니다.
>
...
#### Prophet (총 4종목, 평가금 합 1,696,854,211M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| 228670.KS | 레이 | 158,288,639,079 | 5,263 | 10,720 | 1,696,854,210.9 | +104.0% |
| NE | Noble Corporation | 1 | 66,185 | 69,680 | 0.1 | +5.0% |
| CHWY | Chewy | 1 | 38,966 | 39,924 | 0.0 | +2.0% |
| 4689.T | Z-holdings | 1 | 3,540 | 4,027 | 0.0 | +14.0% |

> Prophet의 158 billion shares 보유는 시뮬상 cash가 너무 커져서 모든 cash로 살 수 있는 양을 매수한 결과 — 현실에서는 시장 충격 / 호가 부족으로 발생 불가능. "절대 도달 불가능한 상한선"임을 정량적으로 보여주는 숫자.
...
미래의 가격 경로를 다 알고 있는 가상의 투자자. **Rolling-interval lookahead** 방식:

- 매월 첫 영업일에 활성 SMIC 리포트 종목별로 **앞으로 한 달 동안 (~다음 리밸런스까지) 의 실현 수익률**을 계산
- 1등 종목의 수익률이 2등 대비 `dominance_threshold` (기본 1.5x) 이상 → **100% 집중**, 아니면 수익률 비례 분산 (캡 `max_weight`)
- 다음 리밸런스에 책 전체를 새 winner로 갈아탐 → 매월 winner를 복리시킴

이렇게 해야 진짜 상한선입니다 (이전 "lifetime peak에서 일괄 청산" 버전은 peak 이후 현금이 놀아서 Weak Prophet에 지는 모순이 있었음). 실거래로 베틸 수 없는 숫자.
