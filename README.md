...
유니버스: SMIC 리서치 추출 원천 **216개 리포트, 203개 심볼**. 이 중 시뮬레이션 통계 유니버스는 **211개 리포트**, 가격 매칭 가능 리포트는 **205개**입니다.

### SMIC 리서치 베이스라인 (페르소나 무관)

페르소나 결과를 보기 전에, SMIC가 발간한 리포트 자체의 사후 통계 — 모든 페르소나의 공통 출발점입니다 (시뮬 윈도우 안에 가격이 매칭된 205개 리포트 기준).

| 지표 | 값 |
|---|---:|
| 총 리포트 수 | **211개** (가격 매칭 205개) |
| 목표가 도달률 | **50.7%** (104/205) |
| 목표가 도달까지 (평균 / 중앙값) | 346일 / 168일 |
| 발간 시점 약속한 목표 상승률 (평균) | **+99.5%** |
| 발간 후 누적 수익률 (평균 / 중앙값) | **+78.2%** / +16.4% |
| 목표가 대비 현재 갭 (평균) | −57.0% |

> 약속한 +99.5% 중 절반(50.7%)만 실제 도달. 그러나 모집단 평균 누적 수익률이 **+78%** 라 → 분포 자체가 손실보다 이익이 큰 right-skew. 모든 페르소나가 +수익으로 끝나는 근본 원인이 여기에 있고, 페르소나 차이는 "이 분포에서 도달 예정인 winner 만 골라잡을 수 있느냐(예언자) vs 분포 전체를 1/N 으로 받아안느냐(추종자)" 의 차이로 좁혀집니다. 자세한 Top 5 winner/loser와 종목별 풀 데이터는 §9-0 / `data/sim/report_stats.json` + `report_performance.csv`.
...
| **Weak Prophet** (6M max-Sharpe) | 2,325.38M | 1억 | +2,225.38M | 136.69% | 22.1% | 1,273 |
| **All-Weather** (벤치마크) | 206.63M | 1억 | +106.63M | 28.45% | 9.5% | 182 |
| **SMIC Follower v2** (손절 추가) | 180.29M | 1억 | +80.29M | 23.09% | 19.0% | 1,961 |
| **SMIC Follower** (1/N 충신) | 137.96M | 1억 | +37.96M | 12.61% | 14.9% | 4,062 |
...
#### Weak Prophet (총 182종목, 평가금 합 2,316.04M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| 322000.KS | 현대에너지솔루션 | 186 | 50,776 | 160,000 | 29.76 | +215.0% |
| 440110.KQ | 파두 | 483 | 21,264 | 60,900 | 29.41 | +186.0% |
| 218410.KQ | RFHIC | 292 | 35,073 | 96,600 | 28.21 | +175.0% |
| 420770.KQ | 기가비스 | 323 | 31,721 | 86,800 | 28.04 | +174.0% |
| 375500.KS | DL이앤씨 | 258 | 40,827 | 102,500 | 26.45 | +151.0% |
...
#### SMIC Follower v2 (총 17종목, 평가금 합 180.06M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| 122640.KQ | 예스티 | 496 | 20,413 | 29,200 | 14.48 | +43.0% |
| 017960.KS | 한국카본 | 246 | 41,177 | 51,100 | 12.57 | +24.0% |
| 000150.KS | 두산 | 9 | 1,113,724 | 1,358,000 | 12.2 | +22.0% |
| 294870.KS | HDC현대산업개발 | 473 | 21,414 | 23,500 | 11.12 | +10.0% |
| 234300.KQ | 에스트래픽 | 3,008 | 3,367 | 3,590 | 10.80 | +7.0% |

#### SMIC Follower (1/N) (총 103종목, 평가금 합 135.23M KRW)

| symbol | 회사 | 수량 | 평단 | 종가 | 평가금(M) | 평가손익 |
|---|---|---:|---:|---:|---:|---:|
| 228670.KQ | 레이 | 233 | 5,263 | 10,720 | 2.50 | +104.0% |
| 368600.KQ | 아이씨에이치 | 1,378 | 893 | 1,468 | 2.02 | +64.0% |
| CRWV | Coreweave | 11 | 119,953 | 174,405 | 1.92 | +45.0% |
| CHGG | Chegg | 1,137 | 1,047 | 1,602 | 1.82 | +53.0% |
| 159010.KQ | 아스플로 | 221 | 5,544 | 7,990 | 1.77 | +44.0% |
...
현재 회귀 테스트는 **81 passing**입니다. 핵심 범위는 시뮬레이션 계약/브로커리지/페르소나/시각화와 PDF 목표가·티커 추출 보정입니다:
...
- 시뮬레이션 통계 유니버스: **211개** 리포트, **205개** 가격 데이터 매칭
- 목표가 도달: **104개 (50.7%)**
- 목표가 도달까지 걸린 시간: 평균 346일, 중앙값 168일
- 발간 후 평균 누적 수익률: **+78.2%** (mean), +16.4% (median)
- 발간 시점 약속한 목표 상승률 평균: **+99.5%**

가격 공급원에서 데이터를 받지 못해 추적하지 않는 종목은 아래 6개입니다. 이들은 `data/warehouse/reports.csv` 에 보고서 메타데이터는 남기되, `daily_prices.csv` 가격 시계열이 없으므로 시뮬레이션 성과 계산에서는 제외됩니다.
...
| 비에이치 | 2023-12-06 | 090460 | 090460.KQ | yfinance 가격 미제공, 미추적 |
...
- **미추적 종목**: yfinance에서 가격 시계열을 받지 못한 6개 심볼은 §9-0 표에 명시하고 성과 계산에서 제외합니다.
