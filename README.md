...
|---|---:|---:|---:|---:|---:|---:|
| **Prophet** (예언자, 상한선) | 22억 4,911M | 1억 | **+21억 4,911M** | **134.94%** | 55.6% | 2,454 |
| **Weak Prophet** (6M 룩어헤드) | 15억 0,374M | 1억 | +14억 0,374M | 114.52% | 33.3% | 884 |
| **All-Weather** (벤치마크) | 2억 2,421M | 1억 | +1억 2,421M | 31.68% | 21.6% | 212 |
| **SMIC Follower** (1/N 충신) | 1억 4,274M | 1억 | +0억 4,274M | 13.94% | 11.5% | 3,238 |
| **SMIC Follower v2** (손절 추가) | 1억 2,597M | 1억 | +0억 2,597M | 9.06% | 9.6% | 1,574 |
...
- **예언자 vs 약한 예언자**: 풀 룩어헤드와 6개월 룩어헤드의 차이는 +30%p IRR — 짧은 룩어헤드도 max-Sharpe를 통해 분산되면 큰 알파.
- **충신 vs 충신 v2**: 손절 룰 3종이 적용되면 MDD는 11.5% → 9.6%로 줄지만, 동시에 IRR도 13.9% → 9.1%로 깎입니다. **'손실 안 보는 대신 상승도 일부 포기'** 의 정량적 지표.
- **올웨더 vs 충신**: 단순 4분할 적립이 SMIC 종목 1/N 적립을 제칩니다 — 추종자가 사라는 대로 사면 분산만으로는 부족하다는 증거.
...
- `.github/workflows/ci.yml` — lint, type, test, schema 검증
- `.github/workflows/price-refresh.yml` — 정기 가격 데이터 업데이트
- `.github/workflows/sync.yml` — SMIC 사이트 인덱스 동기화
- `.github/workflows/budget-check.yml` — Optuna 예산 가드
