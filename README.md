...
서울대 SMIC(서울대학교 학생 투자 동아리) 리서치 PDF를 로컬 아카이브 기준으로 수집·파싱해 추출한 **목표가/티커/발간일 데이터**와, yfinance에서 받은 **KRW 환산 일봉 가격**을 결합해, 다양한 투자자 페르소나의 **계좌 단위 누적 수익률**을 재현하는 Python/data 저장소입니다.
...
- **액면분할/병합 스케일 보정**: 발간일 현재가와 첫 시장 종가의 스케일이 4배 이상 어긋나면, 파이프라인은 리포트 목표가를 같은 배율로 자동 조정해 yfinance raw OHLC 단위와 맞춥니다. 예를 들어 이지바이오 2020-10 리포트는 원문 현재가 77,000원/목표가 103,500원이지만 시장 시계열 첫 종가가 약 5,380원 단위라서 목표가를 약 7,231원으로 환산해 `target_hit`, `target_upside_at_pub`, 전략 매도 로직에 사용합니다. 그래도 `report_performance.csv` 의 개별 행을 확인할 때는 분할/상장 이력을 함께 보세요.
