...
export function PageHero({ eyebrow, title, subtitle, badges, actions, kpis }: Props) {
  ...
  return (
    <header className="rounded-box border border-base-300 bg-base-100 shadow-sm">
      <div className={`grid gap-5 p-5 md:p-6 ${hasRightRail ? 'md:grid-cols-[minmax(0,1fr)_minmax(320px,.85fr)]' : ''}`}>
        <div className="grid content-center gap-3 min-w-0">
          ...
          {subtitle ? <p className="max-w-3xl text-sm leading-relaxed text-base-content/65 md:text-base">{subtitle}</p> : null}
          {badges?.length ? (
            <dl className="flex flex-wrap gap-1.5">
              {badges.map((badge) => (
                <span key={badge.label} className="inline-flex items-center gap-1 rounded-full border border-base-300 bg-base-200/60 px-2.5 py-1 text-xs">
