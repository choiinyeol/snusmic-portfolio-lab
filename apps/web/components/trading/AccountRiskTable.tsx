          ...
          {sorted.map((row) => (
            <tr key={row.id} className="hover:bg-slate-50">
              ...
              <td className="min-w-[130px] max-w-[230px] px-3 py-2">
                {row.kind === 'account' && row.isSelectable ? (
...
function AccountName({ row }: { row: AccountLeaderboardRow }) {
  return (
    <>
...
function kindBadgeClass(kind: AccountLeaderboardRow['kind']): string {
  if (kind === 'account') return 'bg-blue-50 text-blue-700';
  if (kind === 'oracle') return 'bg-amber-50 text-amber-700';
  return 'bg-slate-100 text-slate-600';
}

function objectiveBadge(row: AccountLeaderboardRow) {
...
function downloadStrategies(rows: AccountLeaderboardRow[], filename: string) {
  const headers = [
    'id',
    'kind',
    'label',
