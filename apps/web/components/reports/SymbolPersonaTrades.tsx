...
export function SymbolPersonaTrades({ symbol, episodes, trades, personaLabels }: Props) {
  ...
  const summaryAvgEntry = openEpisodes.length
    ? openEpisodes[0]
    : (selectedEpisodes[0] ?? null);
    ...
    <section id="persona-trades" className="grid gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-base-content/65">전략별로 {symbol}을 어떻게 진입·증액·축소했는지 한 화면에서 비교합니다.</p>
        </div>
        <Link className="btn btn-sm btn-ghost" href={`/portfolio?strategy=${persona}`}>포트폴리오 원장 →</Link>
      </div>
      <SegmentedControl
        ariaLabel="페르소나 선택"
        className="compact"
        ...
        <div className="card-body gap-4 p-5">
          <header className="flex flex-wrap items-baseline justify-between gap-3">
            ...
            <div className="flex flex-col items-end">
              ...
              <strong className={`text-2xl font-black tabular-nums ${totalPnl >= 0 ? 'text-success' : 'text-error'}`}>{formatKrw(totalPnl)}</strong>
              <span className="text-xs text-base-content/55">실현 {formatKrw(realizedPnl)} · 미실현 {formatKrw(unrealizedPnl)}</span>
            </div>
          </header>
          <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <SummaryStat
              label="마지막 매매"
              value={lastTrade ? `${lastTrade.side === 'buy' ? '매수' : '매도'} ${lastTrade.qty?.toLocaleString('ko-KR') ?? '—'}주` : '—'}
          ...
          {lastClosedEpisode ? (
            <div className="rounded-lg border border-base-300 bg-base-200/40 px-4 py-3 text-sm text-base-content/75">
              ...
              <span className="ml-2 text-base-content/55">· {lastClosedEpisode.closeDate ?? '—'} · {formatDays(lastClosedEpisode.holdingDays)} 보유</span>
            </div>
          ) : null}
        </div>
      </article>
      ...
      <article className="card border border-base-300 bg-base-100 shadow-sm">
        <div className="card-body gap-4 p-5">
          <header className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <span className="text-xs text-base-content/55">{selectedEpisodes.length}개 포지션 · 최근 체결 {recentTrades.length}건</span>
          </header>
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="grid gap-2.5" aria-label="포지션 카드">
              {selectedEpisodes.slice(0, 3).map((episode) => {
                ...
                return (
                  <article className="rounded-lg border border-base-300 bg-base-100 p-3" key={`${episode.persona}-${episode.symbol}-${episode.openDate}`}>
                    <div className="flex items-center justify-between text-xs uppercase tracking-[0.16em] text-base-content/55">
                      <strong className="text-base-content">{episode.status === 'open' ? '보유 중' : '청산 완료'}</strong>
              ...
              {selectedEpisodes.length > 3 ? (
                <p className="text-xs text-base-content/55">아래 표에서 전체 {selectedEpisodes.length}개 포지션을 확인할 수 있습니다.</p>
              ) : null}
            </div>
            <div className="grid gap-1.5" aria-label="최근 체결 활동">
              ...
              {recentTrades.map((trade) => (
                <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2.5 rounded-md border border-base-200 px-3 py-2 text-sm" key={`event-${trade.persona}-${trade.date}-${trade.side}-${trade.qty}-${trade.fillPriceKrw}`}>
                  <span className={`h-2 w-2 rounded-full ${trade.side === 'buy' ? 'bg-success' : 'bg-error'}`} />
                  <div className="min-w-0">
                    <strong className="block truncate text-base-content">{trade.side === 'buy' ? '매수' : '매도'} {trade.qty?.toLocaleString('ko-KR') ?? '—'}주</strong>
