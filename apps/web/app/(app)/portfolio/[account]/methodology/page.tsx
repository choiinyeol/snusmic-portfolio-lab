...
export default async function PortfolioMethodologyPage({ params }: { params: Promise<{ account: string }> }) {
  const { account } = await params;
  if (account === NO_ADMITTED_ACCOUNT_PARAM) return <EmptyPortfolioRouteContent />;
  return <PortfolioRouteContent selectedAccount={account} view="methodology" />;
}
