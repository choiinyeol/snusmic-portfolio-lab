  ...
  return (
    <>
      <PageHero
        ...
        actions={<Link className="btn btn-sm btn-ghost" href="/strategies">← 전략 기준</Link>}
        kpis={
          <div className="grid min-w-0 gap-3 sm:grid-cols-2">
            <KpiTile label="실험 선택 종목" value={experiment.positions.length.toLocaleString('ko-KR')} delta={`${experiment.trades.length.toLocaleString('ko-KR')}개 이벤트`} tone="accent" />
            <KpiTile label="실험 누적 수익률" value={formatPercent(finalReturn)} tone={(finalReturn ?? 0) >= 0 ? 'good' : 'bad'} />
          </div>
        }
      />
              ...
              <table className="table table-sm table-zebra">
                ...
                <tbody>{experiment.positions.map((position) => (
                  <tr key={`${position.symbol}-${position.publicationDate}`}>
                    <td>
                      <Link className="link link-hover" href={`/reports/${position.symbol}`}>{position.company || position.symbol}</Link>
                    ...
                    <td className={`tabular-nums ${(position.realizedReturn ?? 0) >= 0 ? 'text-success' : 'text-error'}`}>{formatPercent(position.realizedReturn)}</td>
              ...
              <table className="table table-sm table-zebra">
                ...
                <tbody>{experiment.trades.map((trade) => (
                  <tr key={`${trade.date}-${trade.side}-${trade.symbol}`}>
                    ...
                    <td>
                      <Link className="link link-hover" href={`/reports/${trade.symbol}`}>{trade.company || trade.symbol}</Link>
