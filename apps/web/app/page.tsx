...
export default function DashboardPage() {
  ...
  return (
    <>
        ...
        <div className="card-body grid gap-5 p-5 md:grid-cols-[minmax(0,1fr)_minmax(280px,.6fr)]">
          <div>
            <div className="flex items-center justify-between gap-3">
              ...
              <span
                className={`badge ${trend.tone === 'good' ? 'badge-success' : trend.tone === 'bad' ? 'badge-error' : 'badge-warning'} badge-soft`}
              >
                {trend.mode}
              </span>
            </div>
            <div className="mt-2 text-4xl font-black tabular-nums tracking-[-0.04em] text-base-content md:text-5xl">
              {formatKrw(portfolio.marketValue)}
            </div>
            <div
              className={`mt-1 font-bold tabular-nums ${portfolio.unrealizedPnl >= 0 ? 'text-success' : 'text-error'}`}
            >
            ...
            <div className="mt-3">
              <Sparkline
                values={equitySpark}
                height={56}
                tone={trend.tone === 'bad' ? 'bad' : trend.tone === 'good' ? 'good' : 'accent'}
              />
            </div>
          </div>
      ...
      <Section eyebrow="오늘의 판단" title="먼저 확인할 신호">
            ...
            >
              <div className="card-body gap-3 p-5">
                <div className="flex items-center justify-between gap-2">
                  <span
                    className={`badge badge-soft ${item.tone === 'good' ? 'badge-success' : item.tone === 'bad' ? 'badge-error' : item.tone === 'warn' ? 'badge-warning' : 'badge-primary'}`}
                  >
                    {item.label}
                  </span>
                ...
                <div
                  className={`text-2xl font-black tracking-[-0.04em] ${item.tone === 'bad' ? 'text-error' : item.tone === 'good' ? 'text-success' : item.tone === 'warn' ? 'text-warning' : 'text-primary'}`}
                >
                  {item.metric}
                </div>
              </div>
      ...
      <section className="decision-grid grid gap-4 md:grid-cols-4" aria-label="투자 판단 요약">
        ...
        <KpiTile
          label="90일 계좌 모멘텀"
          value={<span>{formatPercent(trend.return90d)}</span>}
          delta={`120일 ${formatPercent(trend.return120d)}`}
          tone={(trend.return90d ?? 0) >= 0 ? 'good' : 'bad'}
        />
        <KpiTile
          label="상승 포지션 비율"
          value={<span>{formatPercent(trend.positiveBreadth)}</span>}
          delta={`${trend.positiveCount}/${holdings.length} 종목 플러스`}
          tone={(trend.positiveBreadth ?? 0) >= 0.5 ? 'good' : 'warn'}
        />
        <KpiTile
          label="고점 대비 낙폭"
          value={<span>{formatPercent(trend.currentDrawdown)}</span>}
          delta="최근 계좌 수익률 기준"
          tone={(trend.currentDrawdown ?? 0) > -0.1 ? 'good' : 'bad'}
        />
      </section>
          ...
          {holdings.slice(0, 6).map((row, index) => {
            ...
            return (
              <Link
                ...
                className="card card-side border border-base-300 bg-base-100 shadow-sm transition hover:border-primary/40 hover:bg-base-200/40"
              >
                <div className="grid w-14 place-items-center border-r border-base-300 font-mono text-sm font-bold text-base-content/45">
                ...
                <div className="card-body grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center">
                  ...
                  <div className="text-left md:text-right">
                    ...
                    {display.secondary ? (
                      <span className="text-sm text-base-content/50">{display.secondary}</span>
                    ) : null}
                  </div>
                  <div className="text-left md:text-right">
                    <span className={`font-black ${(row.unrealizedReturn ?? 0) >= 0 ? 'text-success' : 'text-error'}`}>
          ...
          <div className="grid gap-3">
            {newestReports.map((report) => (
              <Link
                ...
                className="card border border-base-300 bg-base-100 shadow-sm transition hover:border-primary/40 hover:bg-base-200/40"
              >
                <div className="card-body grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                  ...
                  <div className="flex items-center gap-2 md:justify-end">
                    <span
                      className={`text-lg font-black ${(report.currentReturn ?? 0) >= 0 ? 'text-success' : 'text-error'}`}
                    >
        ...
        >
          <article className="card border border-base-300 bg-base-100 shadow-sm">
            <div className="card-body gap-4">
              <span
                className={`badge badge-soft w-fit ${verdict.tone === 'good' ? 'badge-success' : verdict.tone === 'bad' ? 'badge-error' : 'badge-warning'}`}
              >
                {verdict.headline}
              </span>
