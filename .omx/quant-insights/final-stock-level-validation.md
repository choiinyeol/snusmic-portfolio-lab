# Task 6 — Final stock-level account_id validation

## Verdict

**PASS after task7 materialization fix.** The earlier probe correctly found 0 `stock_rule_*` accounts; the leader fix now materializes the OOS-admitted stock-rule accounts into the existing portfolio artifact surface.

## Required checks

| Requirement | Evidence | Result |
| --- | ---: | --- |
| 10+ `stock_rule_*` accounts in `data/web/portfolio/accounts.json` | `10` | PASS |
| 10+ `stock_rule_*` rows in `data/web/strategies/catalog.json` | `10` | PASS |
| 10+ `stock_rule_*` accounts with trades in `data/web/portfolio/trades.json` | `10` | PASS |
| 10+ `stock_rule_*` accounts with holdings in `data/web/portfolio/holdings.json` | `10` | PASS |
| 10+ `stock_rule_*` accounts with equity series in `data/web/portfolio/equity-daily.json` | `10` | PASS |
...
The accepted stock-rule accounts are now normal portfolio accounts, so `/portfolio/[account_id]` can show:
...
- methodology/buy/sell/risk fields (`data/web/portfolio/accounts.json` and `data/web/strategies/catalog.json`),
